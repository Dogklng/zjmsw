...
$db=new Connection();
$header = mb_substr($order_sn,0,2,'utf-8');
if($header=='YU'){
    $query=$db->query("select * from `app_gift_order` where order_no='$order_sn'");
}else{
    $query=$db->query("select * from `app_order_crowd` where order_no='$order_sn'");
}
...
if($header=='YU'){
    $db->query("update `app_gift_order` set order_status=2,pay_way=2,pay_status=1,pay_time=$time,pay_price=$total_fee where order_no='$order_sn'");
    $query=$db->query("select * from `app_gift_order` where order_no='$order_no'");
    $order=$db->get_one($query);

    $query=$db->query("select * from `app_yun_gift` where id={$order['gift_id']}");
    $info=$db->get_one($query);
    $query=$db->query("select * from `app_yun_right` where level={$info['level']}");
    $info2=$db->get_one($query);
    $time = time();
    $db->query("update `app_yun_apply` (level,nums,shenhe,ruzhu_status,user_id,addtime) values ({$info['level']},{$info2['num']},2,3,{$order['user_id']},$time)");
    $db->query("insert into `app_yun_apply` set level={$info['level']},nums={$info2['num']},shenhe=2,ruzhu_status=3");

    $db->query("update `app_member` set yun_level={$info['level']},user_type=1");
}else{
    $db->query("update `app_order_crowd` set order_status=2,pay_way=2,pay_status=1,pay_time=$time,pay_price=$total_fee where order_no='$order_sn'");
}
