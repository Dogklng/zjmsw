function fileQueueError(file, errorCode, message){
    try {
        var imageName = window.pathimg+"/error.gif";
        var errorName = "";
        ...
        switch (errorCode) {
            case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                imageName = window.pathimg+"/zerobyte.gif";
                break;
            case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                imageName = window.pathimg+"/toobig.gif";
                break;
            case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
    ...
    try {
        switch (errorCode) {
            ...
            case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
                imageName = window.pathimg+"/uploadlimit.gif";
                break;
            default:
                alert(message);
                break;
...
function addImage(src){
    var newElement = "<li><img class='content'  src='" + src + "' style=\"width:100px;height:100px;\">" +
        "<img class='button' src="+window.pathimg+"/fancy_close.png></li>";
...
//alert(src);
    $.ajax({
        ...
        url: delurl, //调用WebService的地址和方法名称组合---WsURL/方法名
        data: "src=" + src,
        success: function(data){
            $('.progressBarStatus').text('删除成功');
