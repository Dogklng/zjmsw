 ...
 * 获取手机短信验证码
 */
function sendmessage($code_type,$phone)
{
	/* $code_type=1;
	$phone=18647639771; */
	...
	if (!in_array($code_type,array('1','2','3','4','5','6','7','8'))) {
 	...
 	if ($code_type == 1 || $code_type == 2) {
	    // 判断一下是否开启短信通道
          $touch = M('touch')->find();
          if ($touch['is_sms'] == 0) {
               return array("status"=>0,"info"=>"短信通道已被关闭,请联系管理员");
          }
  
          $create_at = strtotime(date("Y-m-d"));
          $today_sms_num = M('user_verify')->where(array('code_type' => array('lt', 3)))
            ->where(array('create_at' => array('gt', $create_at)))
            ->count();

          if ($today_sms_num >=  $touch['sms_limit']) {
              return array("status"=>0,"info"=>"短信发送次数已超上限，请联系管理员");
          }

          $latest_sms = M('user_verify')->limit(1)->order('create_at desc')->find();
          $latest_time = time() - $latest_sms['create_at'];
          if ($latest_time <= $touch['sms_rate']) {
              return array("status"=>0,"info"=>"短信发送频次过高，请稍后重试");
          }
    	}	
	...
	switch($code_type)
	{
		...
		case 2:
			...
			break;
        case 3:
            $msg = "（拍卖提醒）你好！第".$num."场拍卖即将于".$start_time."开始，请尽快前往！";
            break;
        case 4:
            $msg = "（拍卖提醒）你好！拍品：".$goods_name." 即将于".$start_time."开始拍卖，请尽快前往！";
            break;
        case 5:
            $msg = "（租赁提醒）你好！商品：".$goods_name." 还有".$start_time."到期！";
            break;
        case 6:
            $msg = "（观展提醒）你好！展览：".$goods_name." 即将于".$start_time."开始，请尽快前往！";
            break;
        case 7:
            $msg = "（拍卖提醒）你好！拍品：".$goods_name." 即将于".$end_time."结束拍卖，请尽快前往！".$url;
            break;
        case 8:
            $msg = "（拍卖提醒）你好！你已成功拍下拍品：".$goods_name."，请尽快前往付款！".$url;
            break;
	}
	
	$post_data = array();
	$post_data['name'] = $config['account'];   //必填参数。用户账号
	$post_data['pwd'] = $config['pswd'];  //必填参数。（web平台：基本资料中的接口密码）
	$post_data['content'] =$msg; //必填参数。发送内容（1-500 个汉字）UTF-8编码
	$post_data['mobile'] = $phone; //手机号码， 多个用英文状态下的 , 隔开
	$post_data['type'] = 'pt'; //必填参数。固定值 pt
	$post_data['sign']='浙江美术网'; //必填参数。用户签名
	
  
	$url='http://api.daiyicloud.com/asmx/smsservice.aspx';
	$o='';
	...
	if($arr['0'] == 0){
		...
		$data['remote_ip'] = $remote_addr;
		$db->add($data);
		return array("status"=>1,"info"=>"短信发送成功,请查收！");
	}else{
		//return array("status"=>0,"info"=>$arr[5]);
		return array("status"=>0,"info"=>$arr);
	}
}
 ...
 * 会员等级
 */
function member_promote($user_id,$integral){
    $member = M('Member')->field('signtime,integral,id,vip,count_integral')->find($user_id);
    $count_integral = $member['count_integral'];
    $MemberPromote = M('integral_grade')->order("type asc")->select();
    $promote_id = 1;
    foreach($MemberPromote as $k => $v){
        if($count_integral > $v['integral']){
            $promote_id = $v['type'];
        }
    }
    if($promote_id != $member['vip']){
        M('Member')->where(array('user_id'=>$user_id))->save(array('vip'=>$promote_id));
    }
}
...
function getpage1($count, $pagesize = 12) {
    $p = new Think\Page($count, $pagesize);
    $p->setConfig('header', '<a class="next" href="/Wxin/Artwork/indexce/">更多</a>');
    $p->setConfig('prev', '上一页');
    $p->setConfig('next', '下一页');
    //$p->setConfig('last', '末页');
    //$p->setConfig('first', '首页');
    $p->setConfig('theme', '%UP_PAGE%&nbsp;&nbsp;%DOWN_PAGE%&nbsp;&nbsp;%HEADER%');
    $p->lastSuffix = false;//最后一页不显示为总页数
    return $p;
}
function getpage2($count, $pagesize = 12) {
    $p = new Think\Page($count, $pagesize);
    $p->setConfig('header', '<a class="next" href="/Wxin/Products/indexce/">更多</a>');
    $p->setConfig('prev', '上一页');
    $p->setConfig('next', '下一页');
    //$p->setConfig('last', '末页');
    //$p->setConfig('first', '首页');
...
function getpage3($count, $pagesize = 8) {
    $p = new Think\Page($count, $pagesize);
    $p->setConfig('header', '<a class="next" href="/Wxin/news/indexce/">更多</a>');
    $p->setConfig('prev', '上一页');
    $p->setConfig('next', '下一页');
    //$p->setConfig('last', '末页');
    //$p->setConfig('first', '首页');
...
function getpage4($count, $pagesize = 10) {
    $p = new Think\Page($count, $pagesize);
    $p->setConfig('header', '<a class="next" href="/Wxin/ArtCommunity/artistListce/">更多</a>');
    $p->setConfig('prev', '上一页');
    $p->setConfig('next', '下一页');
    //$p->setConfig('last', '末页');
    //$p->setConfig('first', '首页');
...
// todo ljj 检查商品
function checkCartNum($goodsid, $num, $skuid=0){
$goods = M("goods")->where(array("id"=>$goodsid ,"is_del"=>0))->find();
$zhekou = 1;
//        if($goods['is_cuxiao']){
...
function PaGe($count,$num,$data){
     //$str = '<a href="" class="page current">1</a> <a href="" class="page">2</a> <a href="" class="page">3</a> <a href="" class="page">4</a> <a href="" class="page">5</a> <a href="" class="page">6</a> <a href="" class="page">7</a> <a href="" class="page">8</a> <a href="" class="page">9</a> <a href="" class="page">10</a> ';
    $page_len = (int)ceil($count/$num);

    if ($page_len==0){
        $page_len=1;
    }

    $str = '<a href="/Home/Pm/result/frist/1" class="">第一页</a>';

    if ($data['frist']==1 || $data==''){
        $now=1;
    }elseif ($data['last']==1){
        $now=$page_len;
    }elseif ($data['prev']==1){
    ...
    if ($page_len<=10){
        if ($now!=1){
            $str .= '<a href="/Home/Pm/result/prev/1/page/'.$now.'" class="prev">上一页 << </a>';
        }
        for ($i=1;$i<=$page_len;$i++){

            if ($i == $now){
                $str .='<a href="/Home/Pm/result/page/'.$i.'" class="page current">'.$i.'</a>';
            }else{
                $str .='<a href="/Home/Pm/result/page/'.$i.'" class="page">'.$i.'</a>';
            }
        }
        if ($now!=$page_len){
            $str .= '<a href="/Home/Pm/result/next/1/page/'.$now.'" class="next">下一页>></a> ';
        }
    }else{
        if ($now <=5){
            if ($now!=1){
            ...
            if ($page_len - $now  >= 5){
                ...
                $str .= '<span class="gap">...</span><a href="/Home/Pm/result/prev/1/page/'.$now.'" class="prev">上一页 << </a>';
                for ($i=$now -4;$i<=$now +5;$i++){
                    if ($i == $now){
                        $str .='<a href="/Home/Pm/result/page/'.$i.'" class="page current">'.$i.'</a>';
                    }else{
                        $str .='<a href="/Home/Pm/result/page/'.$i.'" class="page">'.$i.'</a>';
    ...
    $str .='<a href="/Home/Pm/result/last/1" class="last">最末页</a>';
