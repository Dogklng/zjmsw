...
class ProductsController extends BaseController
{
    ...
    *ZQJ 1101修改
    ...
    public function buy()
    {
        ...
        $res = M('goods')->where(array('id' => $id,'is_del' => 0,'is_sale'=>1,'store'=>array('gt', 0)))->find();
...
//        $res = M('goods')->where(array('id' => $id,'is_del' => 0,'is_sale'=>1,'store'=>1,'cstore'=>0))->find();
        ...
        if ($res['activity_id'] != 0) {
            $rel = M('Goods')->where(array('activity_id'=>$res['activity_id'], 'is_del'=>0,'id'=>array('neq',$id)))->limit(4)->select();
            $store = 1;
            $activity = M('activities')->where(array('id'=>$res['activity_id']))->find();
            if ($activity['type'] == 1) {
...
//        array_pop($data);
    ...
    *加入购物车
    ...
    public function products_dot()
    {
        ...
        $res1 = M('goods_slide')->where(array('goods_id' => $id, 'status' => 1))->field('pic')->select();
        $this->assign('goods_slide', $res1);
        $res = M('goods')->where(array('id' => $id,'is_sale'=>1))->find();
