...
class PayController extends Controller
{
    ...
    private function alipay_config()
    {
        $alipay_config = array(
            ...
            //同步跳转地址，公网可以访问   更新5-2
            'Czreturn_url' => "http://" . $_SERVER['HTTP_HOST'] ."/Wxin/Pay/CzReturnURL",
            //异步通知地址，公网可以访问   更新5-3
            'Cznotify_url' => "http://" . $_SERVER['HTTP_HOST'] ."/Home/Pay/CzNotifyURL",

            //同步跳转地址，公网可以访问   更新5-2
            'return_url' => "http://" . $_SERVER['HTTP_HOST'] ."/Wxin/Pay/ReturnURL",
            //异步通知地址，公网可以访问   更新5-3
            'notify_url' => "http://" . $_SERVER['HTTP_HOST'] ."/Home/Pay/NotifyURL",
    ...
    public function CzReturnURL()
    {
        ...
        if ($result) {
            ...
            if ($order['status'] == 1) {
                ...
                $this->redirect("Wxin/Order/order_error/msg/'支付失败!'");
            }
            ///////////////////////////////////////////////////////更新2//////////////////////////////////////////////////////////////////////////////
        } else {
            //验证失败
            $this->redirect("Wxin/Order/order_error/msg/'充值订单支付失败!'");
        }
    }
    ...
    public function ReturnURL()
    {
        ...
        if ($result) {
            ...
            if ($order['order_status'] == 2) {
                $this->redirect('Wxin/PersonalCenter/myOrder');
            } else {
    ...
    public function Czalipay()
    {
        ...
        $out_trade_no = trim(I("order_no"));
        if (!$out_trade_no) {
            $this->redirect("Wxin/Order/order_error/msg/'无此充值单号不存在!'");
        }
        $order = M("money_order")->where(array('order_no' => $out_trade_no))->find();
        if (!$order) {
            $this->redirect("Wxin/Order/order_error/msg/'无此充值订单!'");
        }
        if ($order['status'] == 1) {
            $this->redirect("Wxin/Order/order_error/msg/'充值订单已经支付过了!'");
        }
        $subject = "浙江美术网支付宝订单支付";
        ...
        vendor("alipay_phone.wappay.service.AlipayTradeService");
        vendor("alipay_phone.wappay.buildermodel.AlipayTradeWapPayContentBuilder");

        //商品描述，可空
        $body = '浙江美术网支付宝订单支付';
        //超时时间 5分钟
        $timeout_express="5m";
/*Class 'AlipayTradeWapPayContentBuilder' not found*/
        ...
        $result=$payResponse->wapPay($payRequestBuilder,$alipay_config['Czreturn_url'],$alipay_config['Cznotify_url']);
