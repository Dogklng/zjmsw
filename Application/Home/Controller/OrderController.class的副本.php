...
class OrderController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($cart_ids) {
            ...
            $c_m = M("cart");
            // 查询是否有活动商品，将商品按照活动分开，并且过滤掉已失效商品
//            $cart_goods_map = array(
//                'id' => array("in", $cart_arr)
//            );
//            $cart_goods = $c_m->field('goods_id')->where($cart_goods_map)->select();
            // 将二维数组转化成一维数组
//            foreach ($cart_goods as $value) {
//                $cart_goods_ids[] = $value['goods_id'];
//            }
//            foreach ($cart_goods_ids as $cart_goods_id) {
//                $goods = M('Goods')->where(array('id' => $cart_goods_id))->find();
//                // 获取所有的活动ID，以及其对应的goods_id
//                $activity_goods[$goods['activity_id']][] = $cart_goods_id;
//            }
//            // 根据不同活动id获取不同活动下的商品
//            foreach ($activity_goods as $activity_id => $activity_good) {
//                if ($activity_id == 0) {
//
//                }
//                $activity = M('Activities')->where(array('is_del'=>0, 'id'=>$activity_id))
...
//            dd($activity_goods);
            ...
            if (count($activity_goods) >= 1) {
                foreach ($activity_goods as $activity_id => $activity_fee) {
                    ...
                    if ($activity['type'] == 1) {
                        // 如果是满减活动
                        if ($activity_fee >= $activity['amount']) {
                            $countprice += ($activity_fee - $activity['discount_amount']);
                        } else {
                            $countprice += $activity_fee;
                        }
                    } elseif ($activity['type'] == 2) {
                        // 如果是折扣活动
                        $countprice += ($activity_fee * ($activity['discount']/100));
            ...
            if (empty($order_list)) {
                $this->redirect("Home/Order/order_error/msg/'无效的购买!'");
            }
            ...
            if ($return['status'] != 1) {
                //print_r($is_groom);die;
                 if($is_groom == 1){
                     $this->redirect("Home/Order/order_error/msg/'{$return['info']}!'");
                 }
//                 else{
//                    $this->error('商品可租天数不足');
//                 }
...
//            print_r();
            ...
            // 判断登录用户
            if (!$userid ) {
                $this->redirect("Home/Order/order_error/msg/'无效的用户!'");
            }
            if (count($cart_arr) <= 0 && $cart_ids) {
                $this->redirect("Home/Order/order_error/msg/'作品数量无效!'");
            }
            // 判断收货地址id
            if (!(intval($address_id) > 0)) {
                $this->redirect("Home/Order/order_error/msg/'无效的收货地址!'");
            }
            // 整理地址
            $address = M("address")->where(array("id" => $address_id, "user_id" => $userid))->find();
            if (!$address) {
                $this->redirect("Home/Order/order_error/msg/'地址信息错误!'");
            }
            ...
            if ($cart_ids) {
                ...
                if ($cart_res['status'] != 1) {
                    ...
                    $this->redirect("Home/Order/order_error/msg/'{$cart_res['info']}!'");
                }
            }
            // 得到total_price
            $goods_m = M("goods");
            ...
            foreach ($order_info as $v) {
                ...
                if (!$goods) {
                    $this->redirect("Home/Order/order_error/msg/'商品已下架!'");
                }
                // 得到商品库存
                ...
                if ($num_res['status'] != 1) {
                    if(!(!$cart_ids && $goods['is_groom'] == 0)){
                        $this->redirect("Home/Order/order_error/msg/'{$num_res['info']}!'");
                    }
                }
                $price = $num_res['price'];
                $oprice = $num_res['oprice'];
           ...
           //判断被推荐商品的推荐店铺id是否存在  ZQJ 20171102  还要存发布该商品的店铺shop_id;
            ...
            if (!$order_res) {
                $this->redirect("Home/Order/order_error/msg/'下单失败!'");
            }
            /*if($coupon != '' && $coupon != NULL){
            ...
            if ($cart_ids) {
                foreach ($cart_arr as $v) {
                    ...
                    if (!$cart_res) {
                        M()->rollback();
                        $this->redirect("Home/Order/order_error/msg/'购物车商品错误!'");
                    }
                }
            }
            M()->commit();
            ...
            if (!$order) {
                $this->redirect("Home/Order/order_error/msg/'无此订单!'");
            }
            if ($order['pay_status'] != 0 && $order['order_status'] != 1) {
                $this->redirect("Home/Order/order_error/msg/'订单无法支付!'");
            }
    ...
    public function balance($order_no){
        ...
        M()->commit();
        $this->ajaxReturn(array('info'=>"/Home/PersonalCenter/myOrder",'status'=>4));
    }
    /**
     *微信支付
     */
    ...
    public function order_status()
    {
        if (IS_POST) {
            ...
            if ($res['order_status'] > 1) {
                $url = U('Home/PersonalCenter/myOrder');
    ...
    public function CzPayOrder(){
        ...
        if(IS_AJAX){
            ...
            switch ($pay_way) {
                case "1";
                    $this->ajaxReturn(array("status" => 1,'order_no' => $order_no, "url" => U('Pay/CzAlipay', array("order_no" =>$order_no))));
                    break;
                case "2";
                    $url = $this->czwx_pay($res);
                    if (!is_string($url)) {
                        $this->ajaxReturn(array("status" => 0, 'order_id' => 0, "info" => "请求超时,请重新下单!"));
                    }
                    $url = IMG_URL.'example/qrcode.php?data=' . $url;
                    $this->ajaxReturn(array("status" => 2, "order_id" => $res, 'url' => $url));
                    break;
                case "3";
    ...
    public function czwx_pay($order_res)
    {
        ...
        if (!$info) {
            ...
            $input->SetTime_expire(date("YmdHis", time() + 600));
            //$input->SetGoods_tag($o_g_r['goods_name'] . $o_g_r['sku_info']);
            $input->SetNotify_url("http://" . $_SERVER['HTTP_HOST'] . "/Home/Pay/czweixin_pay");
            $input->SetTrade_type("NATIVE");
            $input->SetProduct_id($info['order_no']);
            $result = $notify->GetPayUrl($input);
            if ($result['error'] != "") {
    ...
    public function czorder_status()
    {
        if (IS_POST) {
            ...
            $res = $order->where(array('id' => $id))->find();
            if ($res['status'] == 1) {
                $url = U('Home/PersonalCenter/userinfo');
                $this->ajaxReturn(array("status" => 1, 'info' => "", 'url' => $url));
            }
        }
    }
...
//            }
    ...
    private function checkCartNum($goodsid, $num, $skuid=0){
        ...
        if ($goods['activity_id'] != 0) {
            $activity = M('activities')->where(array("id"=>$goods['activity_id']))->find();
            if ($activity['expired_at' > date("Y-m-d H:i:s")]) {
                return array("status" => 0, "info" => "该活动已结束！", "num" => 0);
            }

            if ($activity['started_at' < date("Y-m-d H:i:s")]) {
                return array("status" => 0, "info" => "该活动未开始！", "num" => 0);
            }
        }
        $zhekou = 1;
        if($goods['is_cuxiao']){
