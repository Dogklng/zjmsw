...
class PersonalCenterController extends BaseController
{
	...
	*/
    ...
    public function pendEvaluate()
    {
        ...
        //$map['is_del'] = 0;
        /*$map['is_recommend'] = 1;
        $order = $o_m->where($map)->order("order_time desc")->select();
        foreach($order as $k=>$v){
            $order[$k]['data'] = $v['data'] = $o_g_m->where(array("user_id"=>$map['user_id'],"order_id"=>$v['id'],'status'=>0))->select();
            $order[$k]['count'] = $v['count'] = count($v['data']);
            $order[$k]['index_pic']=$v['data'][0]['goods_spic'];
            $order[$k]['index_name']=$v['data'][0]['goods_name'];
            $order[$k]['price']=$v['data'][0]['goods_price'];
        ...
        $order = $o_g_m->where(array("user_id"=>$map['user_id'],'status'=>0))->select();
        //$sql = "SELECT g.id,g.goods_price,g.goods_nums,g.goods_name,g.goods_cap,i.pay_price,i.order_time,i.order_no FROM app_order_goods g INNER JOIN app_order_info i on g.order_id=i.id  WHERE i.user_id = '{$map['user_id']}' AND g.status = 0 AND i.is_del = 0 AND pay_status >0   order by i.order_time desc";
    ...
    public function goEvaluate()
    {
        ...
        $map['user_id'] = $_SESSION['member_id'];
        $order = $o_g_m->where(array("user_id"=>$map['user_id'],'status'=>0,'id'=>$id))->find();

        $info = $o_m->where(array('id'=>$order['order_id']))->find();
        $this->assign("info", $info);
        //$sql = $sql = "SELECT g.id,g.goods_price,g.goods_nums,g.goods_name,g.goods_cap,i.pay_price,i.order_time,i.order_no FROM app_order_goods g INNER JOIN app_order_info i on g.order_id=i.id  WHERE g.id='{$id}'";
        //$order = M('order_goods')->query($sql);
        dump($order);
        $this->assign("cache", $order);*/
        $this->display();
    ...
    {
        //print_r(date('Y-m-d',strtotime(" 15 day")));
   ...
   /* public function logo_pic()
    {
        if (IS_AJAX) {
            $data = $this->uploadImg();
            $a = $data[0]["savepath"] . substr(1) . $data[0]["savename"];
            $img = substr($a, 1);
            $Info = M('apply')->where(array('user_id' => $_SESSION['member_id'], 'isdel' => 0))->save(array('logo_pic' => $img));
        ...
        $headimg = M('apply')->where(array('user_id'=>$_SESSION['member_id']))->find();
        //dump($headimg);exit;
        $this->assign('headimg',$headimg);
        $this->display();
    }*/
    ...
    */
    ...
    public function hlgl(){
        ...
        if(IS_POST){
            ...
            if($res){
                $this->success('提交成功',U('Home/PersonalCenter/userinfo'));
            }else{
                $this->error('提交失败',U('Home/PersonalCenter/hlgl'));
            }
    ...
    public function crowdAgree(){
        if(IS_AJAX){
            ...
            if($res){
                $this->ajaxReturn(array('status'=>1,'url'=>'/Home/PersonalCenter/crowdIdentity'));
            }else{
    ...
    public function crowdIdentity(){
        if(IS_AJAX){
            ...
            if($res1){
                $this->ajaxReturn(array('status'=>1,'url'=>'/Home/PersonalCenter/crowdAudit'));
            }else{
    ...
    public function newlist(){
        ...
        $show  = $Page->show();//分页显示输出
        $this->assign("page",$show);
        $res = M('news')->where(array('user_id'=>$user_id,'is_del'=>0))->order("addtime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign("res", $res);
        $this->display();
    }
    ...
    public function edituser_vote(){
        ...
        $id = I('get.id');
        if(!$id){
            $this->error("非法操作！",U('/Home/PersonalCenter/newlist/tag/newlist/value/8'));
        }
