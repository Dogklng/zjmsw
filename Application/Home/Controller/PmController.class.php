...
class PmController extends BaseController
{
    ...
    public function chuJia(){
        if (IS_AJAX){
            ...
            // 拍卖廷迟，结束拍卖前一秒，有新的出价则廷迟五分钟
            $remind_time = strtotime('-1 seconds',$pm['end']);
            //$remind_time = strtotime('-20 minutes',$pm['end']);
            if ($data['user_id']==$pm['member_id']){
                $this->ajaxReturn(array('status'=>0,'info'=>"自己的拍品不可竞卖！"));
            }
                ...
                if ($res){
                    ...
                    if ($data['jiage'] >=($res['jiage']+$pm['fudu'])){
                        ...
                        if ($res){
                            // 拍卖廷迟，结束拍卖前一秒，有新的出价则廷迟五分钟
                            if(time()>=$remind_time){
                                $end_time = strtotime('+5 minutes',$pm['end']);
                                $res2 = M('pm')->where(array('id'=>$data['goods_id']))->setField(array('dangqian'=>$data['jiage'],'end'=>$end_time));
                            }else{
                    ...
                    if ($data['jiage'] >=($pm['start_price'])){
                        ...
                        if ($res){
                            ...
                            if(time()>=$remind_time){
                                $end_time = strtotime('+5 minutes',$pm['end']);
                                $res2 = M('pm')->where(array('id'=>$data['goods_id']))->setField(array('end'=>$end_time));
                            }
                            $this->ajaxReturn(array('status'=>1,'info'=>array('err'=>"出价成功！",'dangqian'=>$data['jiage'])));
                        }
    ...
    public function paimaiY()
    {
        ...
        $num = M('pm_num')->where(array('is_del'=>0,'id'=>array('neq','140'),'start_time'=>array('gt',time())))->order('start_time')->select();
        ...
        foreach ($num as $k=>$v){
            ...
            $res = M('pm')->where($map)->order('start')->order('sort desc')->select();
    ...
    public function paimaiS()
    {
        ...
        //拍卖场次
        $num = M('pm_num')->where(array('is_xs'=>0,'is_del'=>0,'id'=>array('neq','140'),'start_time'=>array('lt',time()),'end_time'=>array('gt',time())))->order('start_time')->select();
    ...
    public function paimaiYNum()
    {
        ...
        foreach ($res as $kk=>$vv){
            ...
            $res[$kk]['chujia'] =$ccont;
            $num['zcj'] +=$ccont;
            $price = M('chujia')->where(array('goods_id'=>$vv['id']))->order('id desc')->select();
            if ($price) {
                $res[$kk]['dangqian'] = $price[0]['jiage'];
            }
        }
...
//        var_dump($name);die;
        ...
        foreach ($res as $k=>$v){
            ...
            $res[$k]['chujia'] =$ccont;
            $price = M('chujia')->where(array('goods_id'=>$v['id']))->order('id desc')->select();
            if ($price) {
                $res[$k]['dangqian'] = $price[0]['jiage'];
            }
    ...
    public function paimaiXS()
    {
        ...
        //$num = M('pm_num')->where(array('is_xs'=>1,'is_del'=>0,'start_time'=>array('lt',time()),'end_time'=>array('gt',time())))->order('start_time')->select();
...
//dump($res1);exit;
    ...
    public function pmxq()
    {
        ...
        //调生成订单
        $contab = new ContabController();
        $contab->order_pm();

        $goods_id = I('get.id');

        //$zan=M('zan')->where(array('user_id'=>$_SESSION['member_id'],'goods_id'=>$goods_id))->select();
    ...
    public function pmxq_bzj(){
        if (IS_AJAX){
            ...
            if ($data['user_id']==$res['member_id']){
                $this->ajaxReturn(array('status'=>0,'info'=>"自己的拍品不可参与竞卖！"));
            }
    ...
    public function pmxq_bzjT(){
		...
		if(IS_AJAX){
			...
			$post_data = I('post.');
			$type = $post_data['type'];
			$order_no = 'BDJ' . date('Ymd').substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8);
	...
	//        $order_data["deductible_price"];  //抵扣金额
			switch ($type) {
				case 1: //支付宝
	                ...
	                $this->ajaxReturn(array('info'=>'提交订单成功，请支付','url'=>"/Home/Pm/alipay/style/baoding/type/1/order_no/".$order_no,'status'=>1));
                    break;
				case 2:
				//$this->ajaxReturn(array('info'=>"在线支付未开通",'status'=>0));
    ...
    public function balance($type,$order_no){
        ...
        if($type==1){//支付保定金
            ...
            M()->commit();
            $this->ajaxReturn(array('info'=>"/Home/Pm/pmxq/id/".$order['goods_id'],'status'=>4));
          ...
          M()->commit();
            $this->ajaxReturn(array('info'=>"/Home/PersonalCenter/buyerAddPm/tag/buyerAddPm",'status'=>4));
        }elseif($type==3){//拍品支付
            ...
            M()->commit();
            $this->ajaxReturn(array('info'=>"/Home/PersonalCenter/myOrder/tag/myOrder",'status'=>4));
        }else{
    ...
    /*public function paiSelect(){
        if (IS_AJAX){
            ...
            if ($rz['type']==$type&&$rz['ruzhu_status']==1){
            ...
            $this->ajaxReturn(array('status'=>0, "info"=>'类型选择失败！'));
        }
    }*/
    ...
    public function paiLeimu(){
        if (IS_AJAX){
            ...
            if ($res){
                ...
                //$this->success('选择类目成功！',U('Home/Pm/paiZiliao'));
                //die;
            }
            $this->ajaxReturn(array('status'=>0, "info"=>'选择类目失败！'));
            //$this->error("选择类目失败！",U('Home/Pm/paiLeimu'));
            //die;
        }
        /*$ruzhu_id = I('get.ruzhu_id');

        if ($ruzhu_id){
            $this->assign('ruzhu_id',$ruzhu_id);
            $this->display();
        }
        $this->display();*/
    }
    ...
    public function paiZiliao(){
        if (IS_AJAX){
            ...
            //$this->error("资料添加失败！",U('Home/Pm/paiZiliao'));
            //die;
        }
        //$this->display();
    }
    ...
    public function paiBaozj(){
			...
			$post_data = I('post.');
			$type = $post_data['type'];
			$order_no = 'RZ' . date('Ymd').substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8);
			$user_id = $this->user_id;
			file_put_contents('order.log',$user_id.'|'.$order_no);
	...
	//        $order_data["deductible_price"];  //抵扣金额
			switch ($type) {
				case 1: //支付宝
					//$this->ajaxReturn(array('info'=>"支付宝未开通",'status'=>0));
	                $this->ajaxReturn(array('info'=>'提交订单成功，请支付','url'=>"/Home/Pm/alipay/style/ruzhu/type/1/order_no/".$order_no,'status'=>1));
                    break;
				case 2:
        ...
        if ($res['shenhe']!=2){
            $this->error("审核未通过！",U('Home/Pm/paimaiS'));
            die;
        }
        $this->assign('res',$res);
        $this->display();
	...
	//        $order_data["deductible_price"];  //抵扣金额
			switch ($type) {
				case 1: //支付宝
                    $this->ajaxReturn(array("status" => 1,'order_no' => $order_no, "url" =>"/Home/Pm/alipay/style/pm/type/1/order_no/".$order_no));
					//$this->ajaxReturn(array('info'=>"支付宝未开通",'status'=>0));
	                //$this->ajaxReturn(array('info'=>'提交订单成功，请支付','url'=>"/Home/Pm/alipay/style/pm/type/1/order_no/".$order_no,'status'=>1));
                    break;
				case 2:
                    //$this->ajaxReturn(array('info'=>"银联支付未开通",'status'=>0));
    ...
    public function wx_pay($type,$order_no)
    {
        ...
        $input->SetNotify_url("http://" . $_SERVER['HTTP_HOST'] . "/Home/Pay/weixin_pay");
    ...
    public function order_status()
    {
        if (IS_POST) {
            ...
            if($type==1){
                ...
                if ($res['pay_status'] != 0) {
                    $url = '/Home/Pm/pmxq/id/'.$res['goods_id'];
                ...
                if ($res['pay_status'] != 0) {
                    $url = U('/Home/PersonalCenter/buyerAddPm/tag/buyerAddPm');
                    $this->ajaxReturn(array("status" => 1, 'info' => "", 'url' => $url));
                }
            }else{
                $order = M('order_pm');
                $res = $order->where(array('order_no' => $id))->find();
                if ($res['pay_status'] != 0) {
                    $url = U('/Home/PersonalCenter/buyerXia/tag/buyerXia');
                    $this->ajaxReturn(array("status" => 1, 'info' => "", 'url' => $url));
                }
            }
        }
    ...
    public function PayOrder(){
        if(IS_AJAX){
            ...
            $order = M("order_pm")->where(array('user_id'=>$userId,'order_no'=>$order_no,'order_status'=>1))->find();
            if(!$order){
                $this->ajaxReturn(array('status' => 0, 'info' => '订单无法支付'));
            }
            $saveData['update_time'] = time();
            $res = M("order_pm")->where(array('user_id'=>$userId,'order_no'=>$order_no,'order_status'=>1))->save($saveData);
            if(!$res){
    ...
    protected  function alipay_config($style = '')
    {
		if($style == 'baoding'){
			...
			$returnUrl = "https://" . $_SERVER['HTTP_HOST'] . "/Home/Pm/ReturnBURL";
			$notufyUrl = "https://" . $_SERVER['HTTP_HOST'] . "/Home/Pm/NotifyBURl";
		}else if($style == 'ruzhu'){
			$returnUrl = "https://" . $_SERVER['HTTP_HOST'] . "/Home/Pm/ReturnRURL";
			$notufyUrl = "https://" . $_SERVER['HTTP_HOST'] . "/Home/Pm/NotifyRURl";
		}else if($style == 'pm'){
			$returnUrl = "https://" . $_SERVER['HTTP_HOST'] . "/Home/Pm/ReturnPURL";
			$notufyUrl = "https://" . $_SERVER['HTTP_HOST'] . "/Home/Pm/NotifyPURl";
		}else if($style == 'libao'){
            $returnUrl = "https://" . $_SERVER['HTTP_HOST'] . "/Home/Index/ReturnLURL";
            $notufyUrl = "https://" . $_SERVER['HTTP_HOST'] . "/Home/Index/NotifyLURl";
        }
        $alipay_config = array(
	...
	*/
	public function ReturnBURL(){
		...
		if ($result) {
            ...
            if ($order) {
                $this->redirect('/Home/Pm/pmxq/id/'. $order['goods_id']);
            } else {
	...
	*/
	public function ReturnPURL(){
		...
		if ($result) {
            ...
            if ($order) {
                $this->redirect('/Home/PersonalCenter/buyerXia/tag/buyerXia');
            } else {
                $this->order_error("支付失败！");
            }
            ///////////////////////////////////////////////////////更新2//////////////////////////////////////////////////////////////////////////////
	...
	public function ReturnRURL(){
        ...
        $result = $alipaySevice->check($arr);
		 if ($result) {
            ...
            if ($order) {
                $this->redirect('/Home/Pm/paiSuccess');
            } else {
                $this->order_error("支付失败！");
            }
            ///////////////////////////////////////////////////////更新2//////////////////////////////////////////////////////////////////////////////
