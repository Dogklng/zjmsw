    ...
    */
    ...
    public function fbzc(){
        ...
        if(IS_POST){
            ...
            if($res){
                $this->success('发布成功请等待后台审核',U('/Wxin/PersonalCenter/crowdRec'));
            }else{
                $this->error('发布失败',U('/Wxin/PersonalCenter/crowdRec'));
            }
            $this->redirect('Wxin/PersonalCenter/crowdRec');die;
        }
    ...
    *ZQJ 20171031  法人提交
	...
	public function ysjgl(){       
		if(IS_POST){
			...
			$res = M('artist')->add($data);		
			if($res){
				$this->success('创建成功！',U('Wxin/shop/ysjgl'));
			}else{
				$this->error('创建失败！',U('Wxin/shop/ysjgl'));
			}
			$this->redirect('Wxin/PersonalCenter/userinfo');
		}
		$level = M('artist_level')->select();
