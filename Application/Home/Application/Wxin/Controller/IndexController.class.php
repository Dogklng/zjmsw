...
class IndexController extends BaseController
{
    ...
    public function index()
    {
        ...
        $map['is_shang']=1;
        $map['num_id']=$num['id'];
        $res = M('pm')->where(array('is_shang'=>1,'end'=>array('gt',time())))->order('start')->limit(3)->select();
        if(count($res)<3){
            $limit = 3-count($res);
            if($limit!=3){
        ...
        $res1 = M('goods')->where(array('is_del'=>0,'is_sale'=>1,'store'=>1,'cstore'=>0,'is_show'=>1))->order('sort desc,id desc')->limit(6)->select();
        foreach($res1 as $k=>$v){
        ...
        $banner = M('banner')->where(array('type'=>1,'isdel'=>0))->limit(3)->select();
    ...
    public function PayOrder(){
        if(IS_AJAX){
            ...
            switch ($pay_way) {
                case "1";
                    $this->ajaxReturn(array('info'=>'请支付','url'=>"/Wxin/Pm/alipay/style/libao/type/1/order_no/".$order_no,'status'=>1));
                    break;
                case "3";
    ...
    */
    ...
    protected  function alipay_config($style = '')
    {
        if($style == 'baoding'){
            ...
            $returnUrl = 'http://msw.unohacha.com/Wxin/Pm/ReturnBURL';
            $notufyUrl = 'http://msw.unohacha.com/Wxin/Pm/NotifyBURl';
        }else if($style == 'ruzhu'){
            $returnUrl = 'http://msw.unohacha.com/Wxin/Pm/ReturnRURL';
            $notufyUrl = 'http://msw.unohacha.com/Wxin/Pm/NotifyRURl';
        }else if($style == 'pm'){
            $returnUrl = 'http://msw.unohacha.com/Wxin/Pm/ReturnPURL';
            $notufyUrl = 'http://msw.unohacha.com/Wxin/Pm/NotifyPURl';
        }
        $alipay_config = array(
