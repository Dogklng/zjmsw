...
class BaseController extends Controller
{
	...
	public function __construct()
	{
        ...
        $weixin = is_weixin();
        $this->assign('is_weixin',$weixin?1:0);
...
//
        ...
        $is_Mobile = isMobile();
        if(!$is_Mobile && (MODULE_NAME=='Wxin' || MODULE_NAME=='wxin')){
            ...
            $this->redirect(U('/Home/'.$controller.'/'.$action,$_GET));
        }
    ...
    */
    ...
    public function balance($order_no,$type){
        ...
        if($type=='YU'){
            ...
            $url = "/Wxin/ArtCommunity/crowdFdot/id/".$order['crowd_id'];
        }
        $this->ajaxReturn(array('info'=>$url,'status'=>4));
    }

    public function wxhtml_pay($order_no,$body,$total_fee,$notify_url){
        $headers = array();
        ...
        $appid = "wxd4ffd6bcf1b0c3e7";//微信
        $mch_id = "1493716172";//微信官方的
        $key = "56defccc63de74d71ae919ab1ecae656";//自己设置的微信商家key
