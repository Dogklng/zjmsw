...
class ShopController extends BaseController
{
    ...
    public function index(){
        if(IS_AJAX){
            ...
            if($res1){
                $this->ajaxReturn(array('status'=>1,'info'=>'选择成功!','url'=>'/Home/Shop/selNotice'));
            }else{
    ...
    public function selAgree(){
        if(IS_AJAX){
            ...
            if($res){
                if($type==0){
                    $this->ajaxReturn(array('status'=>1,'url'=>'/Home/Shop/selIdentity'));
                }else{
                    $this->ajaxReturn(array('status'=>1,'url'=>'/Home/Shop/SelIdentityone'));
                }
            }else{
    ...
    public function SelIdentityone(){
        if(IS_AJAX){
            ...
            //$res = M('apply')->where(array('user_id'=>$_SESSION['member_id']))->save($date);
            if($res1){
                $this->ajaxReturn(array('status'=>1,'url'=>'/Home/Shop/ysjgl'));
            }else{
                if($res){
                    $this->ajaxReturn(array('status'=>1,'url'=>'/Home/Shop/ysjgl'));
                }else{
...
//匹配出图片的格式
    ...
    public function selshitiUpload()
    {
        $id = I('get.id');
        if(!empty($id)){
	...
	public function selUpload()
    {
        ...
        $this->assign("attr", $attr);
        $res = M('series')->where(array('isdel'=>'0','cstores'=>0))->order('id asc')->select();
	...
	public function addGoods()
    {
        if (IS_AJAX){
            ...
            M()->commit();
            $this->ajaxReturn(array('status'=>1,'info'=>$data['id']?'修改成功':$data['cstore']));
        }
    }
    ...
    *ZQJ 20171031  法人提交
	...
	public function fbzc(){
	    if(IS_POST){
            ...
            if($res){
                $this->success('新增成功！',U('Home/PersonalCenter/userinfo'));
            }else{
                $this->error('新增失败！',U('Home/PersonalCenter/userinfo'));
            }
        }
    ...
    public function apply(){
        ...
        if($res['ruzhu_status'] == 0){
            $this->ajaxReturn(array('status'=>1,'url'=>'/Home/Shop/index'));
            ...
            $this->ajaxReturn(array('status'=>1,'url'=>'/Home/Shop/selAudit'));
        }elseif($res['ruzhu_status'] == 4){
            $this->ajaxReturn(array('status'=>1,'url'=>'/Home/Shop/selAudit'));
        }else{
            $this->ajaxReturn(array('status'=>2,'url'=>'/Home/Shop/seldpsy'));
        }
    }
