...
class GoodsController extends CommonController {
    ...
    public function goodsList(){
        ...
        $Page  = getpage($count,10);
        $show  = $Page->show();//分页显示输出
        $res = $m->where($map)->order('sort desc,id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function changeStatus(){
        if(IS_AJAX){
            ...
            if($res['is_groom'] == 1){
                $res2 = $m->where(array("id"=>$id))->setField('is_groom',0);
                if($res2){
                    $this->ajaxReturn(array("status"=>2 ,"info"=>"修改成功！"));
                }
            }else{
                $res2 = $m->where(array("id"=>$id))->setField('is_groom',1);
                if($res2){
                    $this->ajaxReturn(array("status"=>1 ,"info"=>"修改成功！"));
                }
            }
