...
class GoodsController extends CommonController {
    ...
    public function goodsList(){
        ...
        /*销售中*/
        $count1=$m->where(array("is_del"=>0, "is_sale"=>1,'shenhe'=>1,'cstore'=>0,'activity_id'=>0))->count();
        $is_sale = I("is_sale");
        if($is_sale==2){
        ...
        /*未销售 下架*/
        $count2=$m->where(array("is_del"=>0, "is_sale"=>0,'shenhe'=>1,'cstore'=>0,'activity_id'=>0))->count();
        if($is_sale==1){
            $map['is_sale'] = intval($is_sale)-1;
         ...
         * 未审核
         */
        $count3=$m->where(array("is_del"=>0, "shenhe"=>0,'cstore'=>0,'activity_id'=>0))->count();
        if($is_sale==3){
         ...
         * 审核通过
         */
        $count4=$m->where(array("is_del"=>0, "shenhe"=>1,'cstore'=>0,'activity_id'=>0))->count();
        if($is_sale==4){
            $map['shenhe'] = intval($is_sale)-3;
         ...
         * 审核拒绝
         */
        $count5=$m->where(array("is_del"=>0, "shenhe"=>2,'cstore'=>0,'activity_id'=>0))->count();
        if($is_sale==5){
            $map['shenhe'] = intval($is_sale)-3;
        ...
        /*商品数量*/
        $count=$m->where($map)->count();
        $countss = $m->where(array('is_del'=>0,'cstore'=>0,'activity_id'=>0))->count();
        $Page  = getpage($count,10);
        $show  = $Page->show();//分页显示输出
