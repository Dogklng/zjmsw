...
class CstoreController extends CommonController
{
    ...
    public function store(){
        ...
        /*销售中*/
        $count1=$m->where(array("is_del"=>0, "is_sale"=>1,'cstore'=>1,'activity_id'=>0))->count();
        $is_sale = I("is_sale");
        if($is_sale==2){
        ...
        /*未销售 下架*/
        $count2=$m->where(array("is_del"=>0, "is_sale"=>0,'cstore'=>1,'activity_id'=>0))->count();
        if($is_sale==1){
            $map['is_sale'] = intval($is_sale)-1;
...
//加的
		/*待审核*/
		$count3=$m->where(array("is_del"=>0,"shenhe"=>0,"cstore"=>1,'activity_id'=>0))->count();
		if($is_sale==3){
			//$map['is_sale'] = intval($is_sale)-1;
			$map['is_del'] = 0;
			$map['shenhe'] = 0;
		...
		/*审核通过*/
		$count4=$m->where(array("is_del"=>0,"shenhe"=>1,"cstore"=>1,'activity_id'=>0))->count();
		if($is_sale==4){
			//$map['is_sale'] = intval($is_sale)-1;
			$map['is_del'] = 0;
			$map['shenhe'] = 1;
		...
		/*审核拒绝*/
		$count5=$m->where(array("is_del"=>0,"shenhe"=>2,"cstore"=>1,'activity_id'=>0))->count();
		if($is_sale==5){
			//$map['is_sale'] = intval($is_sale)-1;
			$map['is_del'] = 0;
			$map['shenhe'] = 2;
...
//
        ...
        $countss = $m->where(array('is_del'=>0,'cstore'=>1,'activity_id'=>0))->count();
    ...
    *zqj 20171106
    ...
    public function changeStatus(){
        if(IS_AJAX){
            ...
            $m    = M("goods");
            $res  = $m->where(array("id"=>$id,'cstore'=>1,'shenhe'=>1,'activity_id'=>0))->find();
            if(!$res){
                $data['status'] = 0;
                $data['info'] = '没有商品';
            ...
            if($res['is_sale'] == 1){
               $res2 = $m->where(array("id"=>$id,'cstore'=>1,'shenhe'=>1,'activity_id'=>0))->setField('is_sale',0);
                if($res2){
                    $data['status'] = 2;
                    $data['info'] = '修改成功';
                   $this->ajaxReturn($data);
            ...
            if($res['is_sale'] == 0){
               $res2 = $m->where(array("id"=>$id,'cstore'=>1,'shenhe'=>1,'activity_id'=>0))->setField('is_sale',1);
                if($res2){
                    $data['status'] = 1;
                    $data['info'] = '修改成功';
                   $this->ajaxReturn($data);
