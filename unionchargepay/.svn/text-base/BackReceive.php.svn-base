...
$db=new Connection();
$query=$db->query("select * from `app_money_order` where order_no='$order_sn'");
...
*/
$db->query("update `app_money_order` set way=3,status=1,pay_time=$time,pay_price=$total_fee where order_no='$order_sn'");
// 删除预约的商品
/*$query = $db->query("select user_id from `app_money_order` where `order_id` = {$order[id]}");*/
/*$goods = $db->fetch_assoc($query);*/
// var_dump($goods);
/*foreach($goods as $v){*/
    $query = $db->query("update `app_member` set wallet=wallet+{$total_fee} where id={$order[user_id]}");
/*}*/
