...
include_once $_SERVER ['DOCUMENT_ROOT'] . '/unionpay/func/SDKConfig.php';
include_once $_SERVER ['DOCUMENT_ROOT'] . '/unionpay/func/secureUtil.php';
include_once $_SERVER ['DOCUMENT_ROOT'] . '/unionpay/func/log.class.php';
...
if(!isset($order_id)){
	echo '<h3><center>订单信ID不能为空</center></h3>';
	exit();
}

$db=new Connection();
$query=$db->query("select * from `app_order_info` where id=$order_id");
...
*/
	...
	if(is_mobile()){
		// 移动跳转
		header('location:http://'.$_SERVER['HTTP_HOST'].'/index.php?g=Wxin&m=Orderlist&a=order_inner&order_id='.$order_id);die;
	}else{
		// pc跳转
		header('location:http://'.$_SERVER['HTTP_HOST'].'/index.php?g=Home&m=Orderlist&a=order_inner&order_id='.$order_id);die;
	}
