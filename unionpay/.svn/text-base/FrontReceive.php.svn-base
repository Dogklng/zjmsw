...
$db=new Connection();
$query=$db->query("select * from `app_product_order` where order_no='$out_trade_no'");
$order=$db->get_one($query);

if(is_mobile()){
	// 移动跳转到订单详情
	header("location:/index.php?g=Wxin&m=Ucenter&a=Jisiorder");
}else{
	// pc跳转到支付完成页面
	//header("location:/index.php?g=Home&m=Orderlist&a=complete&order_id=$order[id]");
    header("location:/Home/Ucenter/Jisiorder.html");
}
die;
