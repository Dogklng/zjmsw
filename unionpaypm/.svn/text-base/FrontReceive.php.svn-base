...
if($type==1){
    $query=$db->query("select * from `app_order_pmding` where order_no='$out_trade_no'");
}elseif($type==2){
    $query=$db->query("select * from `app_ruzhu` where pay_order='$out_trade_no'");
}else{
    $query=$db->query("select * from `app_order_pm` where order_no='$out_trade_no'");
}
